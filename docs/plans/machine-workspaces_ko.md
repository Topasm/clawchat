...
| M2 | 완료 | `1210a6c` + 후속 보완 | 라벨 기본값은 hostname이 아니라 OS별 이름(`My Mac` 등). 라벨과 별개인 영구 `device_id`로 머신을 식별하므로 같은 기본 이름이 작업을 공유하지 않음 |
| M3 | 완료 | `e75d202` + 후속 보완 | 서버 8,000/24,000자, Rust 8/24 KiB로 제한. 로컬 생성 시 첫 snapshot을 즉시 전송하고 claim 시 갱신은 다음 chat/run에 반영. 새로고침 버튼은 해당 실행 머신에서만 표시 |
...
## 8. 기존 프로젝트의 로컬 폴더 설정 간소화 (2026-09-06, v1.4.27)

- 설정에서 이 기기의 작업 실행을 켜고 등록이 완료되면, 프로젝트의
  `어디서 실행하나`에서 해당 기기에만 `찾아보기…`를 제공한다.
- 폴더 선택은 입력값만 바꾼다. `여기서 실행`을 눌러야 폴더를 확인한 뒤
  경로 저장과 실행 기기 선택을 이어서 처리한다. 다른 머신은 기존 수동 설정을 유지한다.
- 기존 bounded context reader로 디렉터리를 확인하므로 README가 없어도 된다.
  쓰기 권한이나 CLI 설치까지 검증하는 기능은 아니다. `~`는 자동 확장하지 않으며
  폴더 선택기 또는 절대 경로를 사용한다.
- 두 서버 요청은 원자적 트랜잭션이 아니다. 두 번째 요청이 실패하면 경로만 저장됐을
  수 있음을 알리고 입력값을 유지한다. 재시도할 수 있으며 자동 롤백하지 않는다.
- 로컬 워커가 꺼져 있으면 설정으로 연결한다. 원격 머신 행이나 웹에는 로컬 폴더
  선택기를 표시하지 않는다. 주기적인 조회가 편집 중인 경로를 덮어쓰지 않는다.
- 실제 Mac/Ubuntu에서 폴더 선택, 접근 거절, 재시도, 앱 재시작 후 바인딩 유지에 대한
  수동 확인은 별도로 필요하다.
