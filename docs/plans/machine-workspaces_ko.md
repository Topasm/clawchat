...
## 9. 데스크톱 경로 설정 진입점 (미출시)

- 프로젝트 헤더와 개별 작업 상세의 Project context에서 동일한 머신·경로 요약을
  사용한다. 경로가 없으면 미설정을 표시하고, 긴 경로는 줄바꿈과 텍스트 선택을 지원한다.
- 프로젝트 실행 설정을 Plan 하단에서 헤더 아래로 이동한다. Activity·Files에서도
  탭 이동 없이 같은 설정을 열며, 설정 화면을 중복 생성하지 않는다.
- 작업 상세에서는 소속 프로젝트 이름과 기본 작업 경로를 바로 보여주고, 변경 버튼으로
  기존 ProjectWorkspaceHosts 편집기를 연다. 채팅을 만들거나 열지 않는다.
- 경로 변경은 프로젝트 전체에 적용된다. 개별 run의 임시 worktree 경로를 바꾸거나
  현재 실행 중인 agent를 이동하는 기능은 아니다. 기존 검증·저장·조회 갱신을 재사용한다.
- Android UI와 서버 계약은 변경하지 않는다. Mac/Ubuntu에서 실제 폴더 선택 및
  저장 후 양쪽 화면의 갱신 확인은 별도로 필요하다.
- 설정 안내에 프로젝트 전체 작업에 적용됨을 명시한다. 경로 저장·머신 변경·경로
  해제·로컬 연결 후에는 해당 프로젝트 상세와 workspace, 프로젝트 목록을 함께
  갱신한다. 관계없는 프로젝트 상세까지 다시 조회하지 않는다.
- 실패 시에도 서버 상태를 다시 조회한다(로컬 연결은 일부만 저장될 수 있음).
  원격 설정 실패도 화면에 표시하고 입력값을 유지해 재시도할 수 있게 한다.
- 서버가 저장 경로를 확인한 뒤에만 편집용 입력값을 해제한다. 이후 외부 변경은
  새 조회값을 표시하되, 저장 전 입력이나 확인되지 않은 저장값은 덮어쓰지 않는다.

## 10. 네이티브 폴더 선택창 멈춤 수정 (미출시)

- `server_select_folder`의 동기 `blocking_pick_folder` 호출을 제거한다.
  async 명령에서 `pick_folder` 콜백 결과를 oneshot 채널로 기다려 UI 스레드를
  막지 않는다. 취소는 기존처럼 null을 반환하고, 콜백 전달 실패는 오류로 반환한다.
- 선택 도중 별도 시간제한으로 창을 강제 종료하지 않는다. 사용자 취소와 선택창 오류
  후에는 화면의 busy 상태가 해제되고 다시 열 수 있어야 한다.
- 소스 회귀 검사는 동기 대기 재도입을 감지한다. UI 모의 테스트는 취소·오류·재열기를
  확인하지만 macOS 네이티브 이벤트 루프를 재현하지는 않는다.
- 새 데스크톱 빌드를 설치한 Mac에서 Desktop 등 다른 폴더로 이동, 권한 창,
  선택·취소·재열기를 확인해야 한다. 웹 UI나 서버만 업데이트해서는 적용되지 않는다.
