...
class AskingOnSecondSkillAI:
    ...
    async def generate_completion(self, *, system_prompt, user_message):
        if "research assistant" in system_prompt:
            ...
            return "Stable research output"
        self.draft_calls += 1
        if "Follow-up instruction:" not in user_message:
            return "NEEDS_INPUT: Formal or casual tone?"
        assert "Previous step (research) output:\nStable research output" in user_message
        return "Draft using the saved research"


class StructuredAskingAI:
    model = "fake-model"
    ...
    async def function_call(self, **_kwargs):
        return {
            "choices": [{
                "message": {
                    "content": None,
...
@pytest.mark.asyncio
async def test_native_ask_user_persists_choices_in_the_thread_card(db_session):
    _project, _todo, task = await create_project_task(db_session, skill_chain=["draft"])
    run = await agent_run_service.create_run(db_session, task, provider="openclaw")
    await db_session.commit()
    ai = StructuredAskingAI()
    ...
    await db_session.refresh(run)
    assert run.status == "waiting_input"
    assert run.progress_message == "Which tone?"
    message = (await db_session.execute(
        select(Message).where(Message.conversation_id == task.conversation_id)
    )).scalar_one()
    metadata = json.loads(message.metadata_json)
    assert metadata["input_options"] == ["Formal", "Casual"]
    ai.generate_completion.assert_not_awaited()
...
async def test_follow_up_answers_the_question_and_finishes(
    ...
    try:
        ...
        assert [m.content for m in thread if m.role == "user"] == ["Casual"]
        assert [
            json.loads(m.metadata_json)["event_type"]
            for m in thread
            if m.metadata_json
        ] == [
...
@pytest.mark.asyncio
async def test_follow_up_resumes_at_the_asking_skill_with_saved_outputs(
    client, auth_headers, db_session
):
    _project, _todo, task = await create_project_task(
        db_session, skill_chain=["research", "draft"]
    )
    run = await agent_run_service.create_run(db_session, task, provider="openclaw")
    await db_session.commit()
    ai = AskingOnSecondSkillAI()
    await agent_task_service.execute_task(
        db_session, task, ai, SimpleNamespace(send_json=AsyncMock()), "user",
        run=run, provider="openclaw",
    )
    ...
    assert run.status == "waiting_input"
    assert task.current_skill_index == 1
    assert json.loads(task.payload_json)["_skill_chain_execution"]["outputs"] == [
    ...
    try:
        ...
        assert resumed.result == "Draft using the saved research"
        assert ai.research_calls == 1
        assert ai.draft_calls == 2
    finally:
        for name in state_names:
            if name in previous:
                setattr(app.state, name, previous[name])
