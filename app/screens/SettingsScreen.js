...
export default function SettingsScreen({ navigation }) {
  ...
  // Server health check
  const [serverHealth, setServerHealth] = useState(null); // null = checking, true = connected, false = disconnected
  const [cacheSize, setCacheSize] = useState('Calculating...');

  // Local state for sliders to avoid laggy re-renders during dragging
  const [localFontSize, setLocalFontSize] = useState(fontSize);
  const [localTemperature, setLocalTemperature] = useState(temperature);
  const [localMaxTokens, setLocalMaxTokens] = useState(maxTokens);

  // Sync local state when store changes (e.g., from import/reset)
  useEffect(() => {
  ...
  const estimateCacheSize = useCallback(async () => {
    // Rough estimate based on AsyncStorage keys
    try {
      ...
      for (const key of keys) {
        ...
        if (value) {
          totalSize += value.length * 2; // rough bytes estimate (UTF-16)
        }
      }
      if (totalSize < 1024) {
  ...
  const handleEditModel = () => {
    Alert.prompt
      ...
      : // Android fallback: simple alert
        Alert.alert(
          'LLM Model',
          `Current model: ${llmModel}\n\nTo change the model, edit the settings JSON via Export/Import.`,
          [{ text: 'OK' }]
        );
  };
  ...
  const copyToClipboard = async (text) => {
    if (ClipboardModule?.setStringAsync) {
      ...
      return true;
    } else if (ClipboardModule?.setString) {
  ...
  const readFromClipboard = async () => {
    if (ClipboardModule?.getStringAsync) {
      return await ClipboardModule.getStringAsync();
    } else if (ClipboardModule?.getString) {
  ...
  const handleExportSettings = async () => {
    try {
      ...
      if (copied) {
        Alert.alert('Exported', 'Settings copied to clipboard as JSON.');
      } else {
        Alert.alert('Settings JSON', json);
      }
    } catch {
  ...
  const handleImportSettings = async () => {
    try {
      ...
      Alert.alert(
        'Import Settings',
        'This will overwrite your current settings with the JSON from your clipboard. Continue?',
        [
          { text: 'Cancel', style: 'cancel' },
          {
            text: 'Import',
            onPress: () => {
              ...
              if (result.success) {
                Alert.alert(
                  'Imported',
                  `Successfully imported ${result.count} settings.`
                );
              } else {
                Alert.alert('Import Failed', result.error || 'Invalid JSON format.');
              }
            },
          },
        ]
  ...
  const handleClearChatHistory = () => {
    Alert.alert(
      'Clear Chat History',
      'This will permanently delete all local chat history. This cannot be undone.',
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Clear',
  ...
  const handleClearCache = () => {
    Alert.alert(
      'Clear Cache',
      'This will clear locally cached data. Your settings and account will not be affected.',
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Clear',
          ...
          onPress: async () => {
            try {
              ...
              const cacheKeys = keys.filter(
                (k) => !k.includes('auth-storage') && !k.includes('settings-storage')
              );
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: () => {
            resetToDefaults();
            Alert.alert('Reset', 'All settings have been restored to defaults.');
          },
        },
      ]
    );
  ...
  const handleSystemPrompt = () => {
    navigation.navigate('SystemPrompt');
  };
  ...
  const sections = [
    {
      ...
      data: [
        ...
        {
          key: 'messageBubbleStyle',
          render: () => (
            <SettingsSegmentedCell
              label="Message Style"
              options={BUBBLE_STYLE_OPTIONS}
              selectedValue={messageBubbleStyle}
              onValueChange={setMessageBubbleStyle}
            ...
            <SettingsNavigationCell
              ...
              detail={
                systemPrompt.length > 30
                  ? systemPrompt.substring(0, 30) + '...'
                  : systemPrompt
              }
              onPress={handleSystemPrompt}
        ...
        },
        {
          ...
          render: () => (
            <SettingsSliderCell
              ...
              onValueChange={setLocalMaxTokens}
              onSlidingComplete={(val) => setMaxTokens(Math.round(val))}
              minimumValue={256}
              maximumValue={8192}
              step={256}
      ...
      data: [
        {
          ...
          render: () => (
            <SettingsButtonCell
              ...
              subtitle="Copy settings JSON to clipboard"
              showSeparator
            />
          ),
        },
        {
          ...
          render: () => (
            <SettingsButtonCell
              ...
              subtitle="Paste settings JSON from clipboard"
              showSeparator
            />
          ),
        },
