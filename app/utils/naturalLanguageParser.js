/**
 * Parse natural language text to extract task/event metadata.
 * @param {string} text - Raw user input
 * @returns {{ title: string, type: 'task'|'event'|'note', dueDate?: Date, startTime?: Date, priority?: string }}
 */
export function parseNaturalInput(text) {
  ...
  const nextDayMatch = cleanTitle.match(/\bnext\s+(monday|tuesday|wednesday|thursday|friday|saturday|sunday)\b/i);
  const inDaysMatch = cleanTitle.match(/\bin\s+(\d+)\s+days?\b/i);
  const monthDayMatch = cleanTitle.match(/\b(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\s+(\d{1,2})\b/i);
...
/**
 * Check if a text input looks like it should create a task or event.
 */
export function shouldAutoCreate(text) {
  const parsed = parseNaturalInput(text);
  return parsed.dueDate || parsed.startTime || parsed.type === 'event' ||
