...
fun eventDate(startTime: String?): LocalDate? = parseEventDateTime(startTime)?.toLocalDate()

private val TIME_FORMAT = DateTimeFormatter.ofPattern("HH:mm")

/** `09:00`, `09:00 – 10:30`, or `All day`. */
fun eventTimeLabel(startTime: String?, endTime: String?, isAllDay: Boolean): String {
    if (isAllDay) return "All day"
    val start = parseEventDateTime(startTime) ?: return ""
    val end = parseEventDateTime(endTime)
    val startLabel = start.format(TIME_FORMAT)
    if (end == null) return startLabel
    return "$startLabel – ${end.format(TIME_FORMAT)}"
}
