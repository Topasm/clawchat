...
export default function SearchPage() {
  ...
  const filters: {
    ...
    { value: 'all', label: translateUi('All') },
    { value: 'tasks', label: translateUi('Tasks') },
    { value: 'events', label: translateUi('Events') },
  ];
  return (
    <div>
      <div className="cc-page-header">
        <div className="cc-page-header__title">{translateUi('Search')}</div>
      ...
      <input
        ...
        type="text"
        placeholder={translateUi('Search tasks and events...')}
        value={query}
      ...
      {!q ? (
        <EmptyState
          ...
          message={translateUi('Type to search across tasks and events')}
        />
      ) : searching ? (
        <div className="cc-empty__message" style={{ padding: 40, textAlign: 'center' }}>
          {translateUi('Searching...')}
        </div>
      ) : totalCount === 0 ? (
        <EmptyState
          icon={<MagnifyingGlassIcon size={20} />}
          message={translateUi('No results for "{{query}}"', { query })}
        />
      ) : (
        <div className="cc-search__results">
          ...
          {results.tasks.length > 0 && (
            <div className="cc-search__section">
              <div className="cc-search__section-title">
                {translateUi('Tasks (')}
