  ...
  return (
    ...
    >
      ...
      <div className="cc-task-flow-node__topline">
        ...
        {proposalSelection && (
          <span className={`cc-task-flow-node__proposal-state cc-task-flow-node__proposal-state--${proposalSelection}`}>
            {proposalSelection === 'fixed' ? 'Goal' : proposalSelection === 'selected' ? 'Included' : 'Excluded'}
          </span>
        )}
        {contextLabel && <span className="cc-task-flow-node__context" title={contextLabel}>{contextLabel}</span>}
      </div>
      <div className="cc-task-flow-node__title" title={todo.title}>{todo.title}</div>
      <div className="cc-task-flow-node__meta">
        {childCount > 0 && <span>{completedChildCount}/{childCount} sub-tasks</span>}
        {dueLabel && <span className={dueLabel === 'Overdue' ? 'cc-task-flow-node__due--overdue' : ''}>{dueLabel}</span>}
        {(todo.depends_on?.length ?? 0) > 0 && <span>{todo.depends_on!.length} dependencies</span>}
        {childCount === 0 && !dueLabel && !(todo.depends_on?.length) && <span>{priority} priority</span>}
      </div>
      ...
      {hasVisibleChildren && !proposalSelection && (
        ...
        >
          <svg viewBox="0 0 16 16" aria-hidden="true">
            <path d={isCollapsed ? 'M6 3l5 5-5 5' : 'M3 6l5 5 5-5'} />
          </svg>
        </button>
      )}
