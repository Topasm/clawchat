  ...
  return (
    ...
    >
      <div className="cc-update-banner__content">
        ...
        {status === 'up-to-date' && <span>ClawChat is up to date.</span>}
        {status === 'available' && (
          <div className="cc-update-banner__message">
            <span>ClawChat v{info?.version} is available.</span>
            {releaseNotes && <span className="cc-update-banner__notes" title={releaseNotes}>{releaseNotes}</span>}
          </div>
        )}
        {status === 'downloading' && (
        ...
        {status === 'ready' && <span>Update ready — restart to apply it.</span>}
        {status === 'restarting' && <span>Installing update and restarting…</span>}
        {status === 'error' && (
          <div className="cc-update-banner__message">
            <span>Update failed.</span>
            <span className="cc-update-banner__notes" title={error ?? undefined}>{error}</span>
          </div>
        )}
      </div>
      ...
      <div className="cc-update-banner__actions">
        {status === 'available' && (
          <button type="button" className="cc-update-banner__btn" onClick={() => void downloadAppUpdate()}>
            Download
          </button>
        )}
        {status === 'ready' && (
          <button type="button" className="cc-update-banner__btn" onClick={() => void installAppUpdate()}>
            Restart Now
          </button>
        )}
        {status === 'error' && (
          <button type="button" className="cc-update-banner__btn" onClick={() => void retryAppUpdate()}>
            Retry
          </button>
        )}
        {canDismiss && (
