...
export default function AttachmentList({ ownerId }: AttachmentListProps) {
  ...
  const handleDelete = async (att: AttachmentResponse) => {
    try {
      ...
      addToast('success', translateUi('Deleted {{filename}}', { filename: att.filename }));
    } catch {
      addToast('error', translateUi('Failed to delete attachment'));
    }
  };
  return (
    <div className="cc-attachments">
      <div className="cc-attachments__title">{translateUi('Attachments')}</div>
      {attachments.map((att) => {
        ...
        return (
          <div key={att.id} className="cc-attachments__item">
            {isImage ? (
              <img className="cc-attachments__preview" src={downloadUrl} alt={att.filename} />
            ) : (
            ...
            <button
              ...
              title={translateUi('Delete attachment')}
            >
              &times;
            </button>
          </div>
